...
public class DataLoader implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws Exception {
        ...
        List<Order> orders = Arrays.asList(
                ...
                new Order(OrderType.FIXTURES, "A4", 8700, "A4용지 구매", ApprovalStatus.WAIT, null, defaultImagePath, "a", null, null, members.get(0), members.get(0).getDepartment()),
                new Order(OrderType.TRANSPORTATION, "KTX", 48000, "판교 출장으로 KTX 결제", ApprovalStatus.WAIT, null, defaultImagePath, "a", null, null, members.get(0), members.get(0).getDepartment()),
                ...
                new Order(OrderType.ENTERTAINMENT, "비비고", 56000, "고객 접대", ApprovalStatus.WAIT, null, defaultImagePath, "a", null, null, members.get(2), members.get(2).getDepartment()),

                new Order(OrderType.ETC, "택시비", 25000, "고객 미팅 후 택시비", ApprovalStatus.WAIT, null, defaultImagePath, "a", null, null, members.get(3), members.get(3).getDepartment()),
                new Order(OrderType.FIXTURES, "문구나라", 30000, "사무실 문구류 구매", ApprovalStatus.WAIT, null, defaultImagePath, "a", null, null, members.get(3), members.get(3).getDepartment()),
                new Order(OrderType.TRANSPORTATION, "택시", 67000, "야근", ApprovalStatus.WAIT, null, defaultImagePath, "a", null, null, members.get(3), members.get(3).getDepartment()),

                new Order(OrderType.FOOD_COSTS, "김밥천국", 24000, "점심 김밥 구매", ApprovalStatus.WAIT, null, defaultImagePath, "a", null, null, members.get(4), members.get(4).getDepartment()),
                new Order(OrderType.FIXTURES, "리바트", 92000, "사무실 의자 구매", ApprovalStatus.WAIT, null, defaultImagePath, "a", null, null, members.get(4), members.get(4).getDepartment()),
                new Order(OrderType.TRANSPORTATION, "버스", 12000, "출장 교통비", ApprovalStatus.WAIT, null, defaultImagePath, "a", null, null, members.get(4), members.get(4).getDepartment()),
    ...
    private String saveDefaultImage() throws IOException {
        // 기본 이미지 파일 설정
        String originalFilename = "default.jpg";
        File dest = new File(imageUploadDir + "/" + originalFilename);

        // 디렉토리 생성
        Files.createDirectories(Paths.get(imageUploadDir));

        // 기본 이미지 복사 (static 디렉토리에 있는 이미지 파일 복사)
        return "C:/Users/SUNJIN/Desktop/SJ_DeptManagament/src/main/resources/static/imgs/" + originalFilename;
    }
}
